...
class dir{
    ...
    public function odir($path){
        ...
        while($this->filee = readdir($this->opendirr)){
            ...
            switch ($this->filee) {
                case '.':
                    echo "<li><a href='javascrip::valid(0)' οnclick='window.history.back(-1);return false;'><img src='img/1.jpg' width='25' height='25'>返回上级</a></li>";
                    break;
                case '..':
                    echo "<li><a href='index.php?url={$this->doc_root}'><img src='img/files.jpg' width='25' height='25'>站点首页</a></li>";
                    break;
                default :
                    /* $this->zsPath  拼接的gbk路径  用于判断是否为文件夹 */
                    $this->zsPath = $this->pathGbk.'/'.$this->fileOutChar;
                    if(is_dir($this->zsPath)){
                        echo "<li><img src='img/files.jpg' width='30' height='30'><a href='index.php?path={$this->path}/{$this->filee}'>".$this->filee."</a></li>";
                    }else {
                        /* 开始获取文件后缀名 */
                        ...
                        //php
                        if(preg_match('/php/i',$this->sub)){
                            $this->sub = 'php.jpg';
                        }
                        //html
						if(preg_match('/html/i', $this->sub)){
                            $this->sub = 'html.png';
                        }
                        //js
                        if(preg_match('/js/i', $this->sub)){
                            $this->sub = 'js.jpg';
                        }
                        //否则使用img目录对于的后缀名
                        echo "<li><img src='img/{$this->sub}' width='30' height='30'><a href='edit.php?f={$this->path}/{$this->filee}'>".$this->filee."</a></li>";
                    }
                    break;
            }
        }
