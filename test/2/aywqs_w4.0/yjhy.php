...
if (file_exists("$zipfile")){} else	{
		echo "<script>alert(\"服务器上不存在数据文件！\");javascript:history.go(-1);</script>";exit;
	}
	  $dir="../$leasindata/data";       
   rmdirs2($dir);
...
class zip
{
 ...
 function ExtractFile($header,$to,$zip){
	...
	function start_unzip($tmp_name,$new_name,$checked){
		...
		if(is_file($upfile[tmp_name])){
			$have_zip_file = 1;
				...
				echo "<script>alert(\"数据还原成功！\");javascript:history.go(-1);</script>";
			}else{
			}
	...
	if(!$have_zip_file){
		echo "<script>alert(\"服务器上不存在数据文件！\");javascript:history.go(-1);</script>";
	}
endif;
?>
  </form>
