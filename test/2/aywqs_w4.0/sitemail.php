...
$del=$_GET['del'];
if ($_REQUEST[del])  {
if(file_exists("$mail/$_REQUEST[del]")) unlink("$mail/$_REQUEST[del]");
echo"<script>alert(\"成功清空信箱！\");</script><meta http-equiv=refresh content=0;url=$thisprog>";exit;
}
?>
...
<table class="leasin" width="95%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr bgcolor="#DAEFE1"> 
          <td height="20">
<center>
  <a href="javascript:history.go(0)">刷新</a>&nbsp;&nbsp;<a href="javascript:void(1)" onClick="window.open ('writemail.php','','top=100,left=0,width=700,height=465,status=no,resizable=yes,scrollbars=yes');">
...
if($checkpower ==super) {
print "<a href=\"mailqk.php\"  OnClick=\"JavaScript: if(confirm('确实要清空所有用户的信箱吗？')) return true; else return false;\"><img src=\"images/fol-over.jpg\" border=0>清空所有的用户的信箱</a>&nbsp;&nbsp;<a href=javascript:void(5) onClick=\"window.open ('superwrite.php','','top=100,left=0,width=700,height=503,status=no,resizable=yes,scrollbars=yes');\"><img src=images/write.gif border=0>发送系统消息</a> ";
}  ?><? 
...
$count=$countnum;
if ($maxpageno<=1) echo "<p>容量/已用（$mailn/<font color=red><b>{$countnum}</b></font>）";
...
if (strlen($detail[4])%2==0) $detail[4]=substr($detail[4],0,34)."...";
else $detail[4]=substr($detail[4],0,33)."...";
}
?>
...
<?
}}}else echo "<br><center><font color=blue>暂无个人消息";
